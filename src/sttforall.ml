...
module DkSignature = Kernel.Signature
...
struct
 ...
 (* Put that somewhere else *)
      ...
      let ty = DkSignature.get_type sg B.dloc cst in
      let _ =
        match DkSignature.get_rules sg B.dloc cst with
        | [] -> Entry.Decl(B.dloc, B.id cst, DkSignature.Static,ty)
        | [r] ->
          assert (r.args = []);
