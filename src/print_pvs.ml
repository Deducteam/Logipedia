...
let rec print_ty_pvs : out_channel -> ty -> unit = fun oc ty ->
  match ty with
...
let rec print_type_list_pvs : out_channel -> _ty list -> unit = fun oc l ->
  match l with
  | [] -> ()
...
let rec print_type_list_b_pvs : out_channel -> _ty list -> unit = fun oc ty ->
    match ty with
  | [] -> () 
  | _ -> Printf.fprintf oc "[";
         print_type_list_pvs oc ty;
    Printf.fprintf oc "]"



let rec print_string_type_list_pvs : out_channel -> string list -> unit = fun oc l ->
  match l with
  | [] -> ()
  | a::[] -> Printf.fprintf oc "%s:TYPE" a
  | a::l -> Printf.fprintf oc "%s:TYPE," a;print_string_type_list_pvs oc l;;




let rec print_prenex_ty_pvs : out_channel -> ty -> unit = fun oc ty ->
  let p = prefix_of_ty ty
      in match p with
  | [] -> () 
  | _ -> Printf.fprintf oc "[";
...
let print__te_pvs : out_channel -> _te -> unit =
  let rec print oc _te =
    match _te with
    | TeVar(x)      ->
        output_string oc (sanitize_name_pvs x)
    | Abs(x,a,t)    ->
       Printf.fprintf oc "LAMBDA(%s:%a):%a" (sanitize_name_pvs x) print__ty_pvs a print t
    | App(t,u)      ->
        Printf.fprintf oc "%a(%a)" print t print u
    | Forall(x,a,t) ->
        Printf.fprintf oc "id(FORALL(%s:%a):%a)" x print__ty_pvs a print t
    | Impl(t,u)     ->
        Printf.fprintf oc "id(%a => %a)" print t print u
    | AbsTy(x,t)    ->
        Printf.fprintf oc "%a" print t
    | Cst(c,l)      ->
      print_name oc c;(match l with
          | [] -> ()
          | _ -> 
        Printf.fprintf oc "[";print_type_list_pvs oc l;Printf.fprintf oc "]"
        )
  in print 

let rec prefix_of_te :  te -> string list = fun te ->
  match te with
  | ForallP(x,te') -> x::(prefix_of_te te')
  | Te(te')       -> []


let rec print_prenex_te_pvs : out_channel -> te -> unit = fun oc te ->
  let p = prefix_of_te te
      in match p with
  | [] -> () 
  | _ -> Printf.fprintf oc "[";
         print_string_type_list_pvs oc p;
...
let rec print_te_pvs : out_channel -> te -> unit = fun oc te ->
  match te with
...
let print_te_ctx : out_channel -> te_ctx -> unit = fun oc ctx ->
  ...
  | (x,_ty)::l ->
    if !with_types then
      begin
        Printf.fprintf oc "%s:%a" x print__ty_pvs _ty;
        List.iter (fun (x,_) -> Printf.fprintf oc ", %s:%a" x print__ty_pvs _ty) l;
        Printf.fprintf oc " "
      end
    else
      begin
...
let print_judgment : out_channel -> judgment -> unit = fun oc j ->
  if !with_types then
    ...
    print__ty_ctx (List.rev j.ty) print_te_ctx (List.rev j.te) print_hyp j.hyp print_te_pvs j.thm
  else
  Printf.fprintf oc "%a; %a ⊢ %a"
    print_te_ctx (List.rev j.te) print_hyp j.hyp print_te_pvs j.thm

let conclusion_pvs : proof -> te = fun prf ->
  let j = 
    match prf with
    | Assume(j)         -> j
    | Lemma(_,j)          -> j
...
let print_proof_pvs : out_channel -> proof -> unit = fun oc prf ->
...
let print_ast_pvs : out_channel -> string -> ast -> unit = fun oc prefix ast ->
...
(*        line "\\noindent";
    ...
    | Definition(n,ty,te) ->
        line "%a %a : %a = %a"
          print_name n print_prenex_ty_pvs ty print_ty_pvs ty print_te_pvs te;
        line "AUTO_REWRITE+ %a" print_name n;
        line ""
    | Axiom(n,te)         -> 
        line "%a %a : AXIOM %a" print_name n print_prenex_te_pvs te print_te_pvs te;
        line "" 
    | Theorem(n,te,prf)   ->
        line "%a %a : LEMMA %a" print_name n print_prenex_te_pvs te print_te_pvs te;
        line "";
